var DraggingElementView = joint.dia.ElementView.extend({
    ...
    initialize: function() {
        ...
        this.model.on('change:position', function(el, chng) {
            ...
            if (chng.y > (this.paper.options.height - el.get('size').height)) {
                el.get('position').y = el.previous('position').y;
            }
    ...
    fly: function(opt) {
        ...
        this.model.transition('angle', -opt.angle, {
            duration: 100 * flightTime
        });
    ...
    pointerup: function(evt, x, y) {
        ...
        this.paper.model.removeCell(this._link.id);
